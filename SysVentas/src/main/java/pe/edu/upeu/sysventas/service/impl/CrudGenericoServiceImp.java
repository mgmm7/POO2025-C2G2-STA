...
public abstract class CrudGenericoServiceImp<T,ID> implements ICrudGenericoService<T, ID> {
    ...
    @Override
    public T uptade(ID id, T t){
        getRepo().findById(id).orElseThrow(()-> new ModelNotFoundException("IO NOT FOUND:"+id));
        return getRepo().save(t);
    }
    @Override
    ...
    public T findById(ID id) {
        return getRepo().findById(id).orElseThrow(()-> new ModelNotFoundException("IO NOT FOUND:"+id));
    }
    @Override
    public void delete(ID id) {
        if(!getRepo().existsById(id)){
            throw new ModelNotFoundException("IO NOT FOUND:"+id);
        }
